...
exports = module.exports = function(settings, logger, async) {
  ...
  function renderTemplate(_templateName, _locals, _headers, transport, _callback) {
  	var templateName = _templateName, 
	    locals = _locals, 
	    headers = _headers,
	    callback = _callback, transporter;
	  
	logger.info('send email _headers: ', _headers);
	logger.info('send email templateName: ', templateName);
	logger.info('send email template settings: ', settings.email.templates);
		...
		function createTemplate(err, template) {
			logger.info('createTemplate', err);
			if (err) throw err
			
			logger.info('createTemplate:: send email templateName: ', templateName);
			logger.info('createTemplate:: send email template settings: ', settings.email.templates);
			
			template(templateName, locals, createEmail(headers));
			
			function createEmail(headers) {
				  return function(err, html, text) {
					logger.info('createEmail', err);
